...
namespace CF.VRent.UPSDK.SDK
{
    public static class SecurityUtil
    {
        ...
        /// <summary>
        /// Use our certificate(pfx) public key to encrypt sensitive data 
        /// </summary>
        /// <param name="dataString">sensitive data</param>
        /// <returns></returns>
        public static string EncryptDataByPublicKey(string dataString)
        {
            X509Certificate2 pc = new X509Certificate2(SDKConfig.SignCertPath, SDKConfig.SignCertPwd, X509KeyStorageFlags.MachineKeySet);

            RSACryptoServiceProvider p = new RSACryptoServiceProvider();
            
            //Use public key to do encrypting and use private key to do decrypting.
            p = (RSACryptoServiceProvider)pc.PublicKey.Key;

            if (dataString != "")
            {
                byte[] enBytes = p.Encrypt(UTF8Encoding.UTF8.GetBytes(dataString), false);
        ...
        /// <summary>
        /// Use our certificate(pfx) private key to decrypt sensitive data 
        /// </summary>
        /// <param name="enDataString">encryted data</param>
        /// <returns></returns>
        public static string DecryptDataByPrivateKey(string enDataString)
        {
            if (enDataString == "")
            {
                return "";
            ...
            var encryptData = Convert.FromBase64String(enDataString);

            X509Certificate2 pc = new X509Certificate2(SDKConfig.SignCertPath, SDKConfig.SignCertPwd, X509KeyStorageFlags.MachineKeySet);
            RSACryptoServiceProvider p = new RSACryptoServiceProvider();

            //Use private key to decrypt data
            p = (RSACryptoServiceProvider)pc.PrivateKey;

            byte[] deBytes = p.Decrypt(encryptData, false);
